...
class WorkingSetStrategy: public PageReplacementStrategy {
  ...
  void handlePageFault(Process & proc, int page) {
    currentTime++;
    cout << "Handling page fault for page " << page << " in process " << proc.process_id << " at time " << currentTime << endl;
    proc.fault_count++;

    if (proc.page_table.find(page) != proc.page_table.end() && proc.page_table[page]) {
      cout << "Page " << page << " already in memory." << endl;
    } else {
      cout << "Page " << page << " not in memory, causing fault." << endl;
    ...
    displayWorkingSetSize(); // Display current working set size after updating
  }
  ...
  void displayWorkingSetSize() {
    cout << "Current Working Set Size: " << accessTimes.size() << ", Min Size: " << minSize << ", Max Size: " << maxSize << endl;
  }
  ...
  int findPageToEvict(Process & proc) {
    ...
    cout << (pageToEvict == -1 ? "No valid page found for eviction." : "Evicting page: " + to_string(pageToEvict) + " with last access at time " + to_string(oldestAccessTime)) << endl;
    return pageToEvict;
  }
  ...
  void evictPage(Process & proc, int page) {
    ...
    cout << "Evicted page: " << page << " from memory." << endl;
  }
...
class LRU_X_Strategy: public PageReplacementStrategy {
    ...
    if (proc.page_table.find(page) != proc.page_table.end() && proc.page_table[page] == 1) {
      ...
      if (proc.page_stack.size() >= proc.total_pages) {
        ...
        if (pageToEvict != -1) {
          ...
          cout << "No page evicted: insufficient access history available." << endl;
        }
      }
      loadPage(proc, page);
    }
  ...
  void updateAccessTime(int page) {
    ...
    cout << "Updated access history for page " << page << ". Current history size: " << pageAccessTimes[page].size() << endl;
  }
  int findPageToEvict() {
    ...
    cout << (pageToEvict == -1 ? "No valid page found for eviction." : "Evicting page: " + to_string(pageToEvict) + " with oldest X-th access at time " + to_string(oldestTime)) << endl;
    return pageToEvict;
  }
...
class LFUStrategy: public PageReplacementStrategy {
  ...
  public: void handlePageFault(Process & proc, int page) override {
    ...
    displayPages(); // Optional: display the current status of pages in memory
  }
  ...
  void evictPage(Process & proc) {
    ...
    cout << "Evicted page: " << lfuPage << endl;
  }
...
class OPTLookaheadStrategy: public PageReplacementStrategy {
  ...
  void handlePageFault(Process & proc, int page) override {
    cout << "Handling page fault for page " << page << " in process " << proc.process_id << endl;
    ...
    if (proc.page_table.count(page) == 0) {
      ...
      cout << "Page " << page << " not found in page table, initializing." << endl;
    }

    // If page is not in memory, a page fault occurs
    if (proc.page_table[page] == 0) {
      cout << "Page " << page << " is not in memory, triggering page fault." << endl;
      proc.fault_count++; // Increment the fault count for this process

      // Check if the memory (represented by the stack) is full
      if (proc.page_stack.size() >= proc.total_pages) {
        cout << "Memory full, stack size: " << proc.page_stack.size() << ", total pages allowed: " << proc.total_pages << endl;
        ...
        if (evicted_page != -1) {
          ...
          cout << "Evicted page: " << evicted_page << " to make room for new page." << endl;
        }
      } else {
        cout << "No eviction necessary, current stack size: " << proc.page_stack.size() << endl;
      }
      ...
      cout << "Page " << page << " is already in memory. No action needed." << endl;
    }
  }
  ...
  int choosePageToEvict(const Process & proc) {
    ...
    // Debug: Output future accesses
    cout << "Future accesses within lookahead window:" << endl;
    for (const auto & fa: futureAccesses) {
      cout << "Page " << fa.first << " next used at step " << fa.second << endl;
    }
    ...
    for (const int page: tempStack) {
      ...
      if (found == futureAccesses.end()) {
        ...
        cout << "Page " << page << " has no future access within the window. Evicting." << endl;
        return page;
    ...
    if (pageToEvict != -1) {
      cout << "Evicting page " << pageToEvict << " with the farthest access at step " << farthestAccess << endl;
    } else {
      cout << "No suitable page found for eviction based on the lookahead logic." << endl;
    }
...
class LIFOStrategy: public PageReplacementStrategy {
  public: void handlePageFault(Process & proc, int page) override {
    ...
    // Page not in memory
    if (proc.page_table[page] == 0) {
      cout << "Page fault for page " << page << " in process " << proc.process_id << endl;
      proc.fault_count++;
...
bool parseInput(istream & input, SimulationConfig & config) {
  ...
  for (int i = 0; i < config.num_processes; ++i) {
    ...
    cout << "Process " << proc.process_id << " will access " << proc.page_access_sequence.size() << " pages.";
    cout << " Total pages allowed in memory: " << proc.total_pages << endl;
    if (proc.total_pages < proc.page_access_sequence.size()) {
      cout << "Eviction expected due to limited memory." << endl;
    } else {
      cout << "No eviction expected, memory capacity meets or exceeds demand." << endl;
    }
...
void simulatePageFaults(Process & proc, PageReplacementStrategy & strategy) {
  cout << "Simulating page faults for process " << proc.process_id << " with " << proc.page_access_sequence.size() << " pages access sequence." << endl;
...
int main() {
  ...
  for (const auto & strategyInfo: strategies) {
    ...
    cout << "RUNNING WITH " << strategyInfo.first << " OUTPUT:\n";
    cout << "TOTAL Faults\n";
    for (const auto & proc: config.processes) {
      cout << "Current Page Fault For " << proc.process_id << ": " << proc.fault_count << endl;
    }
    cout << "***** TOTAL REPLACEMENT: " << total_replacements << " ******\n\n";

  }
  cout << endl << config.lookahead_or_window_size << endl;
